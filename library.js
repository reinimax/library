...
try {
  ...
  info.textContent +=
    " Local storage is disabled. Your changes will not be saved!";
...
function addBook() {
  if (
    ...
    title.checkValidity() &&
    !pages.validity.rangeOverflow &&
    !pages.validity.rangeUnderflow &&
    !pages.validity.stepMismatch &&
    pages.checkValidity()
  ) {
...
function displayLibrary(from) {
  for (let i = from; i < library.length; i++) {
    ...
    cardRead.addEventListener("click", function (e) {
      ...
      cardRead.textContent = library[e.target.dataset.index].read
        ? "Read"
        : "Not read";
      saveLibrary();
    });
